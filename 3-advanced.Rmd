...
Glyph map [@Wickham2012-yr] plots the time series as single glyph on the map and in \proglang{R}, \pkg{GGally} implements the glyph map through the \code{glyphs()} function. The function constructs a data frame with calculated position (\code{gx}. \code{gy}. \code{gid}) of each point on the time series using linear algebra (Equation 1 and 2 in @Wickham2012-yr). The data can then be piped into \code{ggplot} to create the glyph map as: 
