...
The development on spatio-temporal data structure in R has witnessed great changes from the initial \pkg{spacetime} [@spacetime] package. New packages, such as \pkg{stars} [@stars] and \pkg{sftime} [@sftime], have [...]. 
...
Consider the climate data used in the \pkg{cubble} package, \code{climate_aus}. It contains daily precipitation, minimum and maximum temperature for 639 weather stations across Australia in 2020. A `sftime` object can be created from the spatial and temporal aspect of the data as follows:

```{r echo = TRUE}
library(sftime)

climate_spatial <- climate_aus %>% select(id:wmo_id)
climate_temporal <- climate_aus %>% face_temporal() %>% as_tibble()

data_sftime <- left_join(climate_spatial, climate_temporal,  
                         by = "id", multiple = "all") %>% 
  st_as_sftime(coords = c("long", "lat"))
data_sftime
```

An issue here is the object size created of `data_sftime`. whichis `r `object.size(data_sftime)/object.size(climate_aus)` times the size of \code{climate_aus}

```{r echo = TRUE}
object.size(data_sftime)
object.size(climate_aus)
```
