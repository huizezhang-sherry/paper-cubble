...
The \code{cubble} class (\code{cubble_df}) is an S3 class \citep{advr} built on the \code{tibble} class, specifically to organize spatio-temporal data.
...
\midrule()
\endhead
base R & \texttt{{[}}, \texttt{{[}\textless{}-}, \texttt{names\textless{}-}, \texttt{{[}{[}}, \texttt{{[}{[}\textless{}-} \\
tidyverse & \texttt{dplyr\_row\_slice}, \texttt{dplyr\_col\_modify}, \texttt{dplyr\_reconstruct}, \texttt{select}, \texttt{mutate}, \texttt{arrange}, \texttt{filter}, \texttt{summarise}, \texttt{select}, \texttt{slice}, \texttt{rowwise}, \texttt{rename}, \texttt{bind\_cols}, \texttt{group\_by}, \texttt{ungroup}, \texttt{slice\_*}, \texttt{*\_join}, \texttt{relocate}, \texttt{type\_sum} \\
cubble & \texttt{cubble}, \texttt{make\_cubble}, \texttt{new\_spatial\_cubble}, \texttt{new\_temporal\_cubble}, \texttt{make\_spatial\_sf}, \texttt{make\_temporal\_tsibble}, \texttt{face\_temporal}, \texttt{face\_spatial}, \texttt{unfold}, \texttt{key}, \texttt{key\_vars}, \texttt{key\_data}, \texttt{index}, \texttt{index\_var}, \texttt{coords}, \texttt{spatial}, \texttt{fill\_gaps}, \texttt{scan\_gaps}, \texttt{switch\_key}, \texttt{match\_sites}, \texttt{match\_spatial}, \texttt{match\_temporal}, \texttt{geom\_glyph}, \texttt{geom\_glyph\_box}, \texttt{geom\_glyph\_line} \\
\bottomrule()
\end{longtable}
...
\midrule()
\endhead
\texttt{cubble\_df} & \texttt{{[},\ dplyr\_col\_modify,\ key\_data,\ key\_vars,\ key,\ names\textless{}-,\ print,\ tbl\_sum} \\
\texttt{spatial\_cubble\_df} & \texttt{dplyr\_reconstruct,\ dplyr\_row\_slice,\ face\_spatial,\ face\_temporal,\ arrange,\ rename,\ rowwise,\ group\_by,\ ungroup,\ select,\ spatial,\ summarise,\ tbl\_sum,\ unfold,\ update\_cubble} \\
\texttt{temporal\_cubble\_df} & \texttt{arrange,\ dplyr\_reconstruct,\ dplyr\_row\_slice,\ face\_spatial,\ face\_temporal,\ fill\_gaps,\ group\_by,\ ungroup,\ \ rename,\ rowwise,\ scan\_gaps,\ select,\ spatial,\ summarise,\ tbl\_sum,\ unfold,\ update\_cubble} \\
\bottomrule()
\end{longtable}
...
\begin{CodeChunk}
\begin{CodeOutput}
# cubble:   key: id [16], index: date, nested form
# spatial:  [144.5203, -38.144913, 148.4667, -36.128657], Missing CRS!
# temporal: date [date], prcp [dbl], tmax [dbl], tmin [dbl]
  id           long   lat  elev name           wmo_id ts       type   dist group
  <chr>       <dbl> <dbl> <dbl> <chr>           <dbl> <list>   <chr>   [m] <int>
1 ASN00080015  145. -36.2  96   echuca  aerod~  94861 <tibble> clim~ 4034.     4
2 ASN00080091  145. -36.3 105   kyabram dpi     95833 <tibble> clim~ 7460.    10
3 ASN00082042  146. -36.8 502   strathbogie     95843 <tibble> clim~ 6153.     6
4 ASN00084145  148. -37.7  62.7 orbost          95918 <tibble> clim~ 2185.     2
5 ASN00085072  147. -38.1   4.6 east sale air~  94907 <tibble> clim~ 3282.     3
# i 11 more rows
\end{CodeOutput}
\end{CodeChunk}
...
Or use \code{return_cubble = TRUE} to return a cubble object:
...
# temporal: date [date], matched [dbl]
  id          long   lat  elev name  wmo_id type   dist group ts       n_matches
  <chr>      <dbl> <dbl> <dbl> <chr>  <dbl> <chr>   [m] <int> <list>       <dbl>
1 ASN000880~  145. -37.0 290   rede~  94859 clim~ 1838.     1 <tibble>        30
2 406213      145. -37.0  NA   CAMP~     NA river 1838.     1 <tibble>        30
3 ASN000841~  148. -37.7  62.7 orbo~  95918 clim~ 2185.     2 <tibble>         5
4 222201      148. -37.7  NA   SNOW~     NA river 2185.     2 <tibble>         5
5 ASN000850~  147. -38.1   4.6 east~  94907 clim~ 3282.     3 <tibble>        14
# i 11 more rows
\end{CodeOutput}
\end{CodeChunk}
