...
The Global Historical Climatology Network (GHCN) provides daily climate measures from stations across the world. The data \code{historical_tmax} is a subset containing the records of maximum temperature for 236 Australian stations from 1859 through 1969 and provides information also on the latitude, longitude and elevation of each of the stations, extracted using the \texttt{rnoaa} package. The goal of this example is to compare the monthly average maximum temperature between two periods, 1971-1975 and 2016-2020, for stations in Victoria and New South Wales.
...
It is worth to mentioned that some overlapping of the glyphs occurred for a few nearby stations as it can be seen from Figure \ref{fig:basic-manip}. This is a common issue when mapping multiple stations in a geographical area. This issue can be mitigated in some instances by aggregating data to group the data into a larger areas which might be useful to learn about clusters in the data as well as for visualizing those clusters in a glyph map. An continuing discussion of Example \ref{historicaltmax} discussing all those issues and can be found in the Appendix.
