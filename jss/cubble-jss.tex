...
A monthly maximum average temperature is then calculated for both periods where \code{cubble} easily allows to compute this time operation across all the stations. This is a time operation and therefore the nested cubble needs to be first switched to the long cubble form by \code{face_temporal()}:
