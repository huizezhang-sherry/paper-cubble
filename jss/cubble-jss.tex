...
\pkg{spacetime} package with underlying class \pkg{sp} \citep{sp} and
\pkg{xts} \citep{xts} for handling spatial and temporal variables. While
these layouts and classes are still being used and well-maintained
today, they are not compatible with the \pkg{tidyverse} ecosystem
\citep{tidyverse}, which has a huge influence on data analysis in
\proglang{R}, including in the spatial and temporal communities. The
package \pkg{sf} \citep{sf} uses data frame objects to represent spatial
data with an additional \code{geometry} column dedicating to feature
geometries \citep{jr_herring_opengis_2011} (e.g.~POINT, MULTIPOINT,
POLYGON, MULTIPOLYGON etc). For temporal data, \pkg{tsibble}
\citep{tsibble} builds on top of the \pkg{tibble} \citep{tibble} object
