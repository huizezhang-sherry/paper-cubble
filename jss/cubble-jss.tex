...
Spatio-temporal data has a spatial component referring to the location of each observation, a temporal component that is recorded at regular or irregular time intervals, and multiple variables measured at each spatial and temporal value. In data collection, spatio-temporal data is less often collected as a whole piece but fragmented spatial and temporal data for analysts to ensemble into spatio-temporal data. With the {[}integrated{]} data, we hope to fix the time to explore the spatial distribution, or fix the spaital location to explore temporal trends. Ideally, we would be able to carry out each of these tasks easily, and even accommodate complex explorations encompassing both simultaneously. These are all underpinned by a {[}flexible{]} data structure that can represent spatio-temporal data.

The \emph{Representing data} section of the CRAN task view {[}ref{]} lists existing spatio-temporal data representation. Among them, \citet{spacetime} proposes four layouts: full grid layout, sparse grid layout, irregular layout, and trajectory. These layouts are implemented in the \pkg{spacetime} package with underlying class \pkg{sp} \citep{sp} and \pkg{xts} \citep{xts} for handling spatial and temporal variables. The \pkg{stars} package has been a recent implementation to handle spatio-temporal data in an array. The package depends on the \pkg{sf} package and has allowed more access to tidyverse verbs in the workflow.

What remains to be difficult when wrangling spatio-temporal data is that the raw data is not naturally structured in any one of the representation mentioned above and the first difficulty is to cast the data, potentially from multiple sources into one of the data representation. Some examples of these includes map data recorded the shape of a collection of area of interest is provided by the government website; geo-scientific data recorded with longitude and latitude coordinate of locations in the area; and temporal variables indexed by both location and time for the temporal aspect. For spatio-temporal data analysis, merging all these sources can be challenging and joining these tables can go wrong because of the slightest unmatched of the linking variable from different sources.

Another issue is how the concept of tidy data should be applied to spatio-temporal data. The tidy data concept advocates the data to be shaped into 1) one row per observation; 2) one column per variable; and 3) one type of data per table. The long form data is preferred over wide data given the downstream data cleaning and visualisation software i.e.~\pkg{dplyr} and \pkg{ggplot2}. However, the long form can be inefficient to store feature geometries, especially for large multipolygon over long time period, i.e.~sub-daily time series data.
...
{\centering \includegraphics[width=1\linewidth]{/Users/sherryzhang/Documents/research/paper-cubble/figures/illu-matching} 
...
{\centering \includegraphics[width=1\linewidth,height=0.7\textheight]{/Users/sherryzhang/Documents/research/paper-cubble/figures/basic-manip} 
...
{\centering \includegraphics[width=1\linewidth]{/Users/sherryzhang/Documents/research/paper-cubble/figures/basic-agg} 
...
{\centering \includegraphics[width=1\linewidth]{/Users/sherryzhang/Documents/research/paper-cubble/figures/matching-map} 
...
{\centering \includegraphics[width=1\linewidth]{/Users/sherryzhang/Documents/research/paper-cubble/figures/matching} 
