...
`as_cubble()} creates a \pkg{cubble} by supplying the three key components: \code{key} as the spatial identifier; \code{index} as the temporal identifier; and a vector of \code{coords} in the order of longitude first and then latitude. The naming of \code{key} and \code{index} follows the convention in the \pkg{tsibble} package. The cubble created by default is in the nested form. Below is an example of creating a cubble:
...
There are a few information in the \pkg{cubble} header: the name of the \code{key} variable (`id`) and its number (5), bounding box (`[115.97, -32.94, 133.55, -12.42]`), and also the name of variable nested in the \code{ts} column with its type: \code{date [date], prcp [dbl], tmax [dbl], tmin [dbl]}. 
...
Building from an \code{tbl_df} class, \pkg{cubble} has implemented methods for \pkg{dplyr} generics, which includes:
