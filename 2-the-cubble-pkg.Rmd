...
Some functions require time invariant variables to be in the same table as the time varying ones. In cubble, this can be done through `migrate()`, which moves the time invariant variables into the long cubble:
...
Building from an `tbl_df` class, `cubble` has implemented methods for `dplyr` generics, which includes:
...
\pkg{cubble} is also compatible with \pkg{tsibble}. When creating a cubble from a tsibble, Only the `coords` argument need to be specified: 
...
In the created nested form, each element of the list-column `ts` is of `tbl_ts` class so that existing operations on the tsibble class can still be applied:
