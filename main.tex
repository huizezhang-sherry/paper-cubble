...
\pkg{plotly} \citep{plotly}, \pkg{leaflet} \citep{leaflet}, and
\pkg{mapdeck} \citep{mapdeck} uses the underlying javascript library
\pkg{plotly.js}, \pkg{leaflet.js}, and \pkg{mapbox.js}; \pkg{ggiraph}
\citep{ggiraph} for {[}\ldots{]}; \pkg{crosstalk} \citep{crosstalk} has
developed a cross linking between html widgets. There's also more R
implement that produces leaflet extension, i.e.~\pkg{tmap} \citep{tmap},
\pkg{mapview} \citep{mapview}, and \pkg{leafpop} \citep{leafpop}.
