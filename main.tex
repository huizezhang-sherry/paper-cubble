...
the nested form, a cubble is built from a \code{rowwise_df} data frame
...
\textcolor{red}{more beginning} A cubble can be created with
\code{as_cubble()} by supplying the three key components: \code{key} as
the spatial identifier; \code{index} as the temporal identifier; and a
vector of \code{coords} in the order of longitude and latitude. The
naming of \code{key} and \code{index} follows the convention in the
\pkg{tsibble} package.
\textcolor{red}{give some context of the data about to be used: a subset of a larger data etc}
Below is an example of creating a cubble and by default, the cubble
created is in the nested form:
