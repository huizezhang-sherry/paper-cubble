...
sources. widely used in remote sensing
...
sources need to be spatially close to each other and temporally
{[}\ldots{]}. {[} can also think about matching as a form of correlation
between series from a statistics point of view.{]} Cubble has
\texttt{match\_sites()} to match data from multiple sources spatially
and temporally. \texttt{match\_sites()} first matches the two data
source spatially through computing the distance {[}more details on
spatial matching - keep one, max dist{]} and then computes the number of
peaks in the two time series that fall within in a fixed length window.
Figure \ref{fig:illu-matching} illustrates the temporal matching. With
temporal matching, a \texttt{temporal\_var\_to\_match} needs to be first
specified. Another parameter needs to be specified is
\texttt{temporal\_independent} which determines the series for construct
the interval. In Figure \ref{fig:illu-matching}, series A moves before
series a and an interval is constructed after the three highest peaks.
Then the highest three peak from series a is identified and two of them
falls into the interval constructed. There are two matches here at Time
5 and 27.
...
Several arguments are available in \texttt{match\_sites()} to adjust the
matching:

\begin{itemize}
\tightlist
\item
  \texttt{spatial\_n\_keep}: the number of matching to keep
\item
  \texttt{spatial\_dist\_max}: the maximum distance allowed between
  matched pair
\item
  \texttt{temporal\_n\_highest}: the number of highest peak used for
  temporal matching
\item
  \texttt{temporal\_window}: the temporal window
\item
  \texttt{temporal\_min\_match}: the minimum number of peak matching for
  temporal matching
\end{itemize}
...
Water data is collected by the
\href{http://www.bom.gov.au/metadata/catalogue/19115/ANZCW0503900528?template=full}{Bureau
of Meteorology}. This includes \ldots{} water gauges, variables
collected. In particular, water level will interactive with the climate
variable precipitation as rains fall into the river will raise the water
level. Figure \ref{fig:matching-map} shows the available weather station
and water gauges in Victoria, Melbourne.
...
\caption[asdfasd]{asdfasd}\label{fig:matching-map}
\end{figure}
\end{CodeChunk}

From the map, a few water gauges and weather stations are close to each
other and we may be able to observe a spontaneous increase in the
precipitation and water level from the measures, potential with a little
delay in the river gauges. \texttt{match\_sites()} matches one source of
data with another source in both spatial and temporal dimension in a
cubble.
...
column \texttt{dist} and \texttt{group} produced from spatial matching
and \texttt{n\_match} from the number of peak matched temporally.
...
Then plots can be made to inspect the matched pairs on the map or to
view the co-movement of the two series:
...
With the chosen parameter, four pairs of stations stands out, which all
locates in the middle Victoria and the concurrent increase of
precipitation and water level can be observed.
