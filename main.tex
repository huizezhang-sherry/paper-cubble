...
\texttt{cubble}. It maps each combination of latitude and longitude into
a unit.
...
Since each combination of latitude and longitude grid is regarded as a
site, the number of site quickly goes up when mapping on a global scale.

\begin{itemize}
\tightlist
\item
  For a southern hemisphere bounding box of {[}-5, -180, -90, 180{]},
  491,040 id, 8 timestamp, 15MB -\textgreater{} 11 sec
\item
  For an Australia bounding box of {[}-12, 115, -43, 153{]}, 19125 id,
  1,827 times tamp, 133 MB -\textgreater{} 1 mins .
\end{itemize}

This shows a computation on the time dimension is cheaper than the
spatial dimension. Argument \texttt{long\_range} and \texttt{lat\_range}
are available to subset to a looser grid when reading in global level
resolution. \texttt{era5-pressure.nc} has 0.25 degree resolution and to
read in the data in whole degree, use:
...
Here an example in Figure \ref{fig:netcdf} is given to reproduce Figure
19 in \citet{hersbach2020era5} from the ERA5 data. ERA5 data
...
2002-09-22, 2002-09-26, 2002-09-30, and 2002-10-04. Readers who are
...
\caption[sdf]{sdf}\label{fig:netcdf}
\end{figure}
\end{CodeChunk}
