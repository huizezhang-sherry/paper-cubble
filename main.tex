...
\Abstract{
Sptio-temporal data naturally has two levels of observations: site and
time level. {[}problem{]}. In this paper, a new data structure,
\texttt{cubble}, is proposed for manipulating and visualising
...
\texttt{as\_cubble()} creates a \pkg{cubble} by supplying the three key
...
\texttt{migrate()}, which moves the time invariant variables into the
long cubble:
...
Building from an \texttt{tbl\_df} class, \texttt{cubble} has implemented
methods for \texttt{dplyr} generics, which includes:
...
cubble from a tsibble, Only the \texttt{coords} argument need to be
specified. In the created nested form, each element of the list-column
\texttt{ts} is of \texttt{tbl\_ts} class so that existing operations on
the tsibble class can still be applied:
...
local areas. In cubble, \texttt{switch\_key()} can be used to create a
new level of grouping of spatial location by specifying a clustering
...
on the map and in \proglang{R}, \texttt{GGally} implements the glyph map
