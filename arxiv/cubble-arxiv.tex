...
While a cubical conceptual data model is neat to conceptually view the spatio-temporal data, a 3D data representation, or an array, may not be convenient for data wrangling. There are two reasons for this: 1) While arrays are efficient for the computation on numerical values, spatio-temporal data contains more than just numerical variables. Wrangling on character strings and specific datetime classes are common tasks for spatio-temporal data but are not easy with arrays. 2) The \texttt{mutate()} syntax for creating new variables is no longer valid since the operation can no longer define how a created 1D vector should be added to the array.
