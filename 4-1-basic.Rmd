...
Next, the monthly maximum average temperature is  calculated for both periods. This is a *temporal operation* requiring a switch into the long \code{cubble} form using the \code{face_temporal()} function. In addition, a new indicator for the two time periods of interest is created before the calculation of monthly averages:
