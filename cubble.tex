...
A common task in spatio-temporal data analysis is to identify related temporal series from geographically proximate locations. For example, we may want to associate weather stations with nearby river sensors to find the water level series that reflect changes in precipitation. This problem can be generalised by first matching locations across two datasets and then calculating the temporal features of the series. In cubble, locations from two datasets can be matched using the function \texttt{match\_spatial()}. The function calculates the distance matrix of the two datasets and returns \texttt{spatial\_n\_group} groups with the smallest distances. For each group, analysts can include more location with the argument \texttt{spatial\_n\_each} (default to 1 for one-on-one matching).
