...
This problem can be tackled by first matching spatial locations across two datasets and then calculating the temporal features of the multiple series. In cubble, locations from two datasets can be matched using the function \texttt{match\_spatial()}. The function calculates the distance matrix of the two datasets and returns \texttt{spatial\_n\_group} groups with the smallest distances. For each group, analysts can include more locations with the argument \texttt{spatial\_n\_each} (default to 1 for one-on-one matching).

The temporal matching then calculates a similarity score between the time series of spatially matched pairs using the function \texttt{match\_temporal()}. The temporal matching requires two identifiers: one for separating each spatially matched group: \texttt{match\_id} and one for separating the two data sources: \texttt{data\_id}. Matching between different variables can be specified using the \texttt{temporal\_by} argument, similar to the \texttt{by} syntax from dplyr's \texttt{*\_join}.
...
The similarity score is computed by a matching function which can be customized to any desired time series feature. The matching function takes two time series as a list and returns a single numerical value. By default, cubble uses a simple peak matching algorithm (\texttt{match\_peak}) to count the number of peaks in two time series that fall within a specified time window.

\hypertarget{interactive-graphics}{%
\subsection{Interactive graphics}\label{interactive-graphics}}

The cubble workflow can be easily incorporated into an interactive graphics pipeline (e.g., \citet{buja1988elements}; \citet{buja1996interactive}; \citet{sutherland2000orca}; \citet{xie2014reactive}; \citet{cheng2016enabling}). The cubble data structure simplifies data pre-processing and prepares the ingredient for interactive graphics. Specifically, the spatial and temporal cubble correspond to the spatial and temporal visualisation, such as a map or a time series plot, that can be linked. Figure \ref{fig:illu-interactive} illustrates the linking mechanism between a map and multiple time series in the \pkg{crosstalk} \citep{crosstalk} package. When a user selects a location on the map as shown on panel (a), the corresponding site is highlighted. This selection activates a row in the spatial cubble, which is then connected to the temporal cubble, resulting in the selection of all observations with the same ID as depicted in panel (b). Consequently the temporal cubble highlights the corresponding series in the time series plot displayed in panel (c). The linking can also be initiated from the time series plot by selecting points on the time series graph. This action selects rows with the same ID in the temporal cubble and the corresponding row in the spatial cubble so that points can be highlight on the map.
...
{\centering \includegraphics[width=1\linewidth]{/Users/cookd/students_PhD/Sherry/paper-cubble/figures/glyphmap-1} 
...
{\centering \includegraphics[width=1\linewidth]{/Users/cookd/students_PhD/Sherry/paper-cubble/figures/matching-1} 
...
{\centering \includegraphics[width=1\linewidth]{/Users/cookd/students_PhD/Sherry/paper-cubble/figures/netcdf-1} 
