...
For example, we might want to find a group of close stations and then use score functions to describe time series features between the series. In cubble, matching locations based on distance can be performed using the function `match_spatial()` which finds  matched groups in two cubble objects by first fiding the closests pairs and.
...
and matching time series features for selected locations`match_temporal()`. The `match_spatial()` function :
