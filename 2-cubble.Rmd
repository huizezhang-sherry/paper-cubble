...
```{r echo = TRUE}
cb_long2 <- cb_long %>% make_temporal_tsibble() 
all.equal(cb_long2, res_long)
```
...
```{r echo =TRUE}
#res_nested %>% sf::st_transform(crs = "EPSG:3857")
```
...
```{r echo = TRUE}
cb_nested2 <- cb_nested %>% make_spatial_sf() 
all.equal(cb_nested2, res_nested)
```
