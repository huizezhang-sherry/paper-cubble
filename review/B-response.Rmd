...
Section 2 has been reworked to combine the original Section 2 and 3. It now focuses more on the inner work of the package: the cubble class and its attributes, object creation and coercion, functions and methods implemented, compatibility with `tsibble` and `sf`, and comparison with other existing spatio-temporal packages (`stars` and `sftime`).
...
All the S3 methods have been modified accordingly. Methods that behave consistently in the nested and long cubble are implemented in the `cubble_df` class, otherwise, separately in the `spatial_cubble_df` and `temporal_cubble_df` classes.
...
-   The key checking in creating cubble from separate spatial and temporal components has been moved to a separate function: `check_key()`.
...
**\textcolor{purple}{We have reworked Section 2 to describe the cubble class and its associated functionality. Additionally, the text in the paper has been aligned with the package vignette. }**
...
**\textcolor{purple}{The data structure proposed in the paper is an infrastructure development to carry the spatio-temporal data analysis and visualisation is a crucial part of the analysis that delivers the results in visuals. We add a paragraph at the beginning of the now Section 4 to explain the purpose of these examples.}**
...
**\textcolor{purple}{Your suggestion has been adopted, see the Software section in the summary of the changes for details. The code in the example (now Section 4.1) is now simpler.}**
...
**\textcolor{purple}{Adopted. Now when applying face\_spatial to a spatial cubble, the cubble will be printed as it is with a message signalling it is already in the nested form. see the reprex below. Same for face\_temporal on a temporal cubble object.}**
...
It would be useful to print a few rows of the spatial attribute of the "long" object. **\textcolor{purple}{It doesn't make sense to print spatial rows if it is a subclass temporal\_cubble\_df. But they do get printed in the subclass spatial\_cubble\_df and you can extract the spatial variables with spatial() as explained in Section 2.1. }**

Section 3.6 is missing `tidyr` package which is especially relevant here given its nesting and unnesting operations <https://tidyr.tidyverse.org/articles/nest.html> **\textcolor{purple}{The tidyr functions are not applicable to a cubble. The idea of tidy data is underlying the cubble class as explained in Section 2.1.}**

More comments inlined in the pdf. **\textcolor{purple}{Done with the section rework.}**
...
-   doubled/missing words/none correct sentences:
    ...
    -   ". . . using 2020 measurements using match_sites() function." \**textcolor{purple}{to "... using the match\_sites() function"}**
...
-   Typos:
    ...
    -   The polar vortex, signalled by the high *speicfic* humidity, splits into two on 2002-09-26 and *further-s- split_s\_* into four on 2002-10-04. **\textcolor{purple}{to "specific", "further splits"}**
...
**\textcolor{purple}{The data historical\_tmax contains daily weather station data for the two periods: 1971-1975 and 2016-2020 and amounts to 3.2MB, which is not suitable to include in the package given its size. The tmax\_hist data, which was included in the package, is now removed, given its similarity to the climate\_aus data, which is more useful. We provide the script to generate historical\_tmax in the Appendix and a copy of the data is available in the paper GitHub repository: https://github.com/huizezhang-sherry/paper-cubble.}**

The manuscript cannot be reproduced, as the reproducible-script.R (part of the submission to JSS) only uses a subset, but does not provide a separate result file that would allow to compare the reproduced outputs with the desired output for the subset. **\textcolor{purple}{A result file in HTML is submitted this time for result comparison.}**
