...
The package \pkg{GGally} [@ggally] has implemented glyph maps through the \code{glyphs()} function. The function constructs a data frame with calculated position (\code{gx}, \code{gy}, \code{gid}) of each point on the time series using linear algebra (Equations 1 and 2 in @Wickham2012-yr). The data can then be piped into ggplot to create the glyph map as:
